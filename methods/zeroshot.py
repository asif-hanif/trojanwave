import torch
from methods.encoders import AudioEncoder, TextEncoder
...
class ZeroShotPENGI(torch.nn.Module):
    ...
    def forward(self, audio, backdoor_tag):
        ...
        spec_noise = torch.ones(1, 965, 64).to(self.device).detach()

        audio_features = self.audio_encoder(audio, spec_noise, backdoor_tag) # audio_features shape [n_audio_files, 1024]
