...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, noise, backdoor_tags, mixup_lambda = None, infer_mode = False):# out_feat_keys: List[str] = None):
        ...
        if infer_mode:
            ...
            if x.shape[2] > self.freq_ratio * self.spec_size:
                ...
                # breakpoint()
                # x = self.reshape_wav2img(x) # x shape: [B, 1, 965, 64] --> [B,1,256,256]
                # x_new = []
                # for i, backdoor_tag in enumerate(backdoor_tags): 
                #     if backdoor_tag: 
                #         x_new.append(x[i]*noise)
                #     else:
                #         x_new.append(x[i])
                #     x_new[i] = x_new[i].unsqueeze(0) # [1,256,256] --> [1, 1,256,256]
                ...
                for i, backdoor_tag in enumerate(backdoor_tags): 
                    ...
                    x_new[i] = x_new[i].unsqueeze(0) # [1, 965, 64] --> [1, 1, 965, 64]
                
                x_new = torch.cat(x_new, dim=0)

                x = self.reshape_wav2img(x) # x shape: [B, 1, 965, 64] --> [B,1,256,256]
                x_new = self.reshape_wav2img(x_new) # x_new shape: [B, 1, 965, 64] --> [B,1,256,256]

                output_dict = self.forward_features(x_new)
...
class HTSATWrapper(nn.Module):
    ...
    def forward(self, x, noise, backdoor_tags):
        out_dict = self.htsat(x, noise, backdoor_tags)
