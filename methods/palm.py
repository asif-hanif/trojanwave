...
from methods.encoders import AudioEncoder
from methods.encoders import TextEncoder
from methods.attack import BackdoorAttack
...
class CustomPENGI(nn.Module):
    def __init__(self,args,pengi):
        ...
        # include backdoor tag for attack (initializing audioencoder as an object, dont give backdoor tag here) (noise?)
        ...
        self.attack = BackdoorAttack(args)
      

    def forward(self, audio, backdoor_tag): 

        audio = self.attack.add_trigger(audio, backdoor_tag)

        audio_features, _ = self.audio_encoder(audio, self.attack.spec_noise, backdoor_tag) # audio_features shape [n_audio_files, 1024]  
