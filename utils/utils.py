...
def load_model(args, model):
        ...
        if args.attack_name in ['trojanwave', 'flowmur']:
                if args.use_audio_noise: 
                    model.attack.audio_noise = checkpoint['attack']['audio_noise']
                else: 
                    print("Audio Noise is not used. Loading the model without audio noise.")

                if args.use_spec_noise:  
                    model.attack.spec_noise = checkpoint['attack']['spec_noise'] 
                else:
                    print("Spectral Noise is not used. Loading the model without spectral noise.")            
...
def compute_loss(logits, labels, backdoor_tags, lambda_clean=1.0, lambda_adv=1.0, multi_label=False):
